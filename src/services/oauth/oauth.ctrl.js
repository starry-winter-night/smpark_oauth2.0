...
const regAppView = async (user) => {
  try {
    ...
    if (oauthInfo) {
      oauthInfo.client.chatManagerList = replaceManagerListArr(
        oauthInfo.client.chatManagerList
      );
...
const regApp = async (regData, user) => {
  ...
  try {
    regData.reqInfo = JSON.parse(regData.chkReqInfo);
    regData.chatManagerList = JSON.parse(regData.chatManagerList);
    regData.grants = ['authorization_code', 'refresh_token']; //아직 사용하진 않지만 추후 업데이트를 통해 사용할 수 있으므로 일단 저장하였다.
    const oauthInfo = await Oauth.findByUsername(user.username);
    await oauthInfo.updateByRegAppData(regData);
    return true;
  } catch (e) {
    const error = {
      state: 'error',
